{{-- resources/views/reports/member-statement.blade.php --}}
{{-- FIXED: All variables properly bound and null-safe --}}
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <table class="header-table">
        <tr>
            <td style="width:65%;">
                ...
                <div class="member-name">{{ $member->user->name ?? 'N/A' }}</div>
                <div class="society-name">{{ $society->name ?? 'N/A' }}</div>
            </td>

            <td class="header-right-cell" style="width:35%;">
                <div class="period-label">Reporting Period</div>
                <div class="period-value">{{ $period->format('F Y') ?? 'N/A' }}</div>
                <div class="cycle-value">Cycle: {{ $cycle->name ?? 'N/A' }}</div>

                <div style="margin-top:8px;color:#7fa8d4;font-size:9px;">
                    {{ $cycle->start_date->format('d M Y') ?? 'N/A' }}
                    &ndash;
                    {{ $cycle->end_date->format('d M Y') ?? 'N/A' }}
                </div>
            </td>
        </tr>
    </table>
...
<div class="page-body">
    ...
    {{-- Loan Status --}}
    @php
        $activeLoan = $memberData['active_loan'] ?? null;
    @endphp
    ...
    @if($activeLoan)
        <div class="loan-banner warn">
            ...
            <strong>M {{ number_format($activeLoan->outstanding_balance ?? 0, 2) }}</strong>
            due on
            <strong>{{ $activeLoan->due_date->format('d M Y') ?? 'N/A' }}</strong>

            Status:
            <span class="badge {{ ($activeLoan->status ?? 'active') == 'overdue' ? 'badge-overdue' : 'badge-active' }}">
    ...
    @php
        ...
        $totalInterestMonth = $memberData['total_interest_month'] ?? 0;
        $sharePercent = $memberData['share_percent'] ?? 0;
    ...
    <table class="kpi-table">
        <tr>
            <td class="kpi-cell">
                <div class="kpi-card {{ $totalContributedMonth > 0 ? 'green':'red' }}">
                    <div class="kpi-label">Contributed</div>
                    <div class="kpi-value">
                        M {{ number_format($totalContributedMonth, 2) }}
                    </div>
            ...
            <td class="kpi-cell">
                <div class="kpi-card {{ $totalPenaltiesMonth > 0 ? 'red':'green' }}">
                    <div class="kpi-label">Penalties</div>
                    <div class="kpi-value">
                        M {{ number_format($totalPenaltiesMonth, 2) }}
                    </div>
                    <div class="kpi-sub">This month</div>
                </div>
            </td>
            ...
            <td class="kpi-cell">
                <div class="kpi-card {{ $totalInterestMonth > 0 ? 'gold':'green' }}">
                    <div class="kpi-label">Interest Paid</div>
                    <div class="kpi-value">
                        M {{ number_format($totalInterestMonth, 2) }}
                    </div>
                    <div class="kpi-sub">This month</div>
                </div>
            </td>
            ...
            <td class="kpi-cell">
                <div class="kpi-card gold">
                    ...
                    <div class="kpi-value">{{ $sharePercent }}%</div>
                    <div class="kpi-sub">
                        M {{ number_format($shareValue, 2) }} est.
                    </div>
                </div>
            </td>
        </tr>
    ...
    @php
        $contributions = $memberData['contributions'] ?? collect();
    @endphp
    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($contributions as $i => $txn)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td>{{ $txn->transaction_date->format('d M Y') ?? 'N/A' }}</td>
                    <td>{{ $txn->notes ?? '-' }}</td>
                    <td class="right">
                        M {{ number_format($txn->amount ?? 0, 2) }}
                    </td>
                </tr>
            @empty
                <tr class="empty-row">
    ...
    @php
        ...
        $totalInterestCycle = $memberData['total_interest_cycle'] ?? 0;
    @endphp
    ...
    <div class="share-box">
        ...
        <table class="share-row-table">
            <tr>
                <td class="share-row-label">Total Contributed</td>
                <td class="share-row-value">
                    M {{ number_format($totalContributedCycle, 2) }}
                </td>
            </tr>
            ...
            <tr>
                <td class="share-row-label">Interest Paid</td>
                <td class="share-row-value gold">
                    M {{ number_format($totalInterestCycle, 2) }}
                </td>
            </tr>
            ...
            <tr>
                <td class="share-row-label">Your Share</td>
                <td class="share-row-value">
                    {{ $sharePercent }}%
                </td>
            </tr>
            ...
            <tr>
                <td class="share-row-label" style="font-size:12px;color:#fff;">
                    Estimated Value
                </td>

                <td class="share-row-value large">
                    M {{ number_format($shareValue, 2) }}
                </td>
            </tr>
        </table>
    </div>
...
<div class="footer-band">
    <table class="footer-table">
        <tr>
            <td>
                <strong style="color:#0f2d5e;">
                    {{ $member->user->name ?? 'Member' }}
                </strong><br>

                {{ $society->name ?? 'Society' }}
                &mdash;
                Member Statement {{ $period->format('F Y') ?? 'Report' }}
            </td>
